...
namespace GalaxyOrbit4Launcher.SchemeHandler
{
    /// <summary>
    ...
    public class FolderSchemeHandlerFactory : ISchemeHandlerFactory
    {
        ...
        /// </summary>
        public static Func<string, string> GetMimeTypeDelegate = (s) => { return Cef.GetMimeType(s); };
        ...
        protected virtual IResourceHandler Create(IBrowser browser, IFrame frame, string schemeName, IRequest request)
        {
            try
            {
                ...
                {
                    return ResourceHandler.ForErrorMessage(string.Format("HostName {0} does not match the expected HostName of {1}.", uri.Host, hostName), HttpStatusCode.NotFound);
                }

                //Get the absolute path and remove the leading slash
                string asbolutePath = uri.AbsolutePath.Substring(1);
                ...
                string filePath = WebUtility.UrlDecode(Path.GetFullPath(Path.Combine(rootFolder, asbolutePath)));
                ...
                if (filePath.StartsWith(rootFolder, StringComparison.OrdinalIgnoreCase) && File.Exists(filePath))
                {
                    ...
                    string mimeType = GetMimeTypeDelegate(fileExtension);
                    FileStream stream = new FileStream(filePath, FileMode.Open, FileAccess.Read, resourceFileShare);
