...
namespace GO2FlashLauncher.Script.GameLogic
{
    internal class MainScreen
    {
        ...
        {
            Point? result = bmp.FindImage(Path.GetFullPath("Images\\cancel.png"), 0.8);
            if (result == null)
            {
                for (int i = 2; i < 7; i++)
                {
        ...
        {
            Point? result = bmp.FindImage(Path.GetFullPath("Images\\warehouse.png"), 0.8);
            if (result == null)
            {
                for (int i = 2; i < 9; i++)
                {
        ...
        {
            Point? result = bmp.FindImage(Path.GetFullPath("Images\\harvest.png"), 0.8);
            if (result == null)
            {
                for (int i = 2; i < 3; i++)
                {
        ...
        {
            Bitmap crop = await bmp.Crop(new Point(0, 0), new Size(150, 500));
            Point? result = crop.FindImage(Path.GetFullPath("Images\\EZReward.png"), 0.95);
            if (result == null)
            {
                for (int i = 2; i < 3; i++)
                {
        ...
        {
            Point? result = bmp.FindImage(Path.GetFullPath("Images\\disconnected.png"), 0.8);
